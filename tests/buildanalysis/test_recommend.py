...
class TestCategoryAndModule:
    def test_module_and_category_columns_present(self):
        interventions = [
            _make_intervention(effort_days=1),
            _make_intervention(effort_days=5),
        ]
        result = build_pareto_frontier(interventions)
        assert "module" in result.columns
        assert "category" in result.columns

    def test_category_quick_win(self):
        result = build_pareto_frontier([_make_intervention(effort_days=1.5)])
        assert result.iloc[0]["category"] == "quick_win"

    def test_category_medium(self):
        result = build_pareto_frontier([_make_intervention(effort_days=5)])
        assert result.iloc[0]["category"] == "medium"

    def test_category_strategic(self):
        result = build_pareto_frontier([_make_intervention(effort_days=15)])
        assert result.iloc[0]["category"] == "strategic"

    def test_category_boundary_at_2_days(self):
        result = build_pareto_frontier([_make_intervention(effort_days=2.0)])
        assert result.iloc[0]["category"] == "medium"

    def test_module_nullable(self):
        result = build_pareto_frontier([_make_intervention()])
        assert result.iloc[0]["module"] is None or pd.isna(result.iloc[0]["module"])

    def test_empty_has_module_and_category_columns(self):
        result = build_pareto_frontier([])
        assert "module" in result.columns
        assert "category" in result.columns
...
class TestScoreDependencyInterventions:
    def _make_cp_result(self, path, total_time_s=45.0):
        return CriticalPathResult(
            path=path,
            total_time_s=total_time_s,
            target_slack=pd.DataFrame({"cmake_target": path, "slack_ms": [0.0] * len(path)}),
    ...
    def test_basic(self, diamond_graph, diamond_timing):
        cp = self._make_cp_result(["A", "B", "D"])
        edge_list = pd.DataFrame(
            {
                ...
                "cmake_visibility": ["PUBLIC", "PRIVATE", "PUBLIC", "PRIVATE"],
            }
        )
        result = score_dependency_interventions(
            diamond_graph,
            diamond_timing,
            cp,
            edge_list,
    ...
    def test_top_n_limits_results(self, diamond_graph, diamond_timing):
        cp = self._make_cp_result(["A", "B", "D"])
        edge_list = pd.DataFrame(
            {
                "source_target": ["A", "A", "B", "C"],
                ...
                "is_direct": [True, True, True, True],
                "dependency_type": ["link"] * 4,
                "cmake_visibility": ["PUBLIC", "PUBLIC", "PUBLIC", "PUBLIC"],
            }
        )
        result = score_dependency_interventions(
            diamond_graph,
