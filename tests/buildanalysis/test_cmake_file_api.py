"""Tests for buildanalysis.cmake_file_api against static fixture reply data."""
...
@pytest.fixture
def codemodel(tmp_path) -> CodeModel:
    ...
    api_reply = tmp_path / ".cmake" / "api" / "v1" / "reply"
    ...
    return parse_reply(tmp_path, FIXTURE_DIR)
...
class TestParseReply:
    def test_loads_all_fixture_targets(self, codemodel: CodeModel):
        ...
        assert EXPECTED_TARGETS.issubset(target_names), f"Missing targets: {EXPECTED_TARGETS - target_names}"
...
class TestReconstructCompileCommand:
    ...
    def test_returns_none_for_no_compile_group(self, codemodel: CodeModel):
        ...
        pytest.skip("No sources without compile group found on INTERFACE target")
