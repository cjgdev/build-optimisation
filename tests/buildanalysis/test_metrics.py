"""Tests for buildanalysis.metrics."""

import pandas as pd
import pyarrow as pa

from buildanalysis.metrics import (
    EDGE_LIST_SCHEMA,
    FILE_METRICS_SCHEMA,
        ...
        df = pd.DataFrame(
            {
                ...
                "compiler_parse_time_ms": [10, 20, 5, 5, 5],
                "compiler_template_instantiation_ms": [5, 10, 2, 2, 2],
                "compiler_codegen_time_ms": [8, 15, 3, 3, 3],
                "compiler_optimization_time_ms": [12, 25, 4, 4, 4],
                "compiler_total_usr_ms": [50, 100, 25, 25, 25],
                "compiler_total_sys_ms": [10, 20, 5, 5, 5],
        ...
        df = pd.DataFrame(
            {
                ...
                "compiler_parse_time_ms": pd.Series([], dtype=float),
                "compiler_template_instantiation_ms": pd.Series([], dtype=float),
                "compiler_codegen_time_ms": pd.Series([], dtype=float),
                "compiler_optimization_time_ms": pd.Series([], dtype=float),
                "compiler_total_usr_ms": pd.Series([], dtype=float),
                "compiler_total_sys_ms": pd.Series([], dtype=float),
...
class TestSafeSum:
    def test_normal_series(self):
        assert safe_sum(pd.Series([10, 20, 30])) == 60

    def test_empty_series(self):
        assert safe_sum(pd.Series([], dtype=float)) == 0

    def test_all_na(self):
        assert safe_sum(pd.Series([None, None], dtype=float)) == 0


class TestSafeMax:
    def test_normal_series(self):
        assert safe_max(pd.Series([10, 20, 30])) == 30

    def test_empty_series(self):
        assert safe_max(pd.Series([], dtype=float)) == 0

    def test_all_na(self):
        assert safe_max(pd.Series([None, None], dtype=float)) == 0


class TestMapFileToTarget:
    def test_hit(self):
        index = {"/src/a.cpp": "target_a", "/src/b.cpp": "target_b"}
        assert map_file_to_target("/src/a.cpp", index) == "target_a"

    def test_miss(self):
        index = {"/src/a.cpp": "target_a"}
        assert map_file_to_target("/src/unknown.cpp", index) is None
