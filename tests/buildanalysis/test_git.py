...
class TestCochange:
    def test_file_level(self, synthetic_git_log):
        result = compute_cochange_matrix(synthetic_git_log, level="file", min_cochanges=2)
    ...
    def test_max_commit_size_filter(self, synthetic_git_log):
        ...
        result = compute_cochange_matrix(synthetic_git_log, level="file", min_cochanges=1, max_commit_size=2)
    ...
    def test_min_cochanges_filter(self, synthetic_git_log):
        result = compute_cochange_matrix(synthetic_git_log, level="file", min_cochanges=5)
    ...
    def test_jaccard_range(self, synthetic_git_log):
        result = compute_cochange_matrix(synthetic_git_log, level="file", min_cochanges=1)
...
class TestFileToTargetMap:
    def test_dedup_preserves_first(self):
        fm = pd.DataFrame(
            {
                "source_file": ["/a.cpp", "/a.cpp", "/b.cpp"],
                "cmake_target": ["t1", "t2", "t3"],
            }
        )
        result = compute_file_to_target_map(fm)
        assert result["/a.cpp"] == "t1"
        assert result["/b.cpp"] == "t3"
        assert len(result) == 2
