...
class TestSummariseContributorFiles:
    def test_aggregation(self):
        ...
        alice_types = [
            r for r in result if r["contributor"] == "alice@example.com" and r["source_file"].endswith("types.cpp")
        ]
        assert len(alice_types) == 1
        assert alice_types[0]["commit_count"] == 2

        bob_types = [
            r for r in result if r["contributor"] == "bob@example.com" and r["source_file"].endswith("types.cpp")
        ]
