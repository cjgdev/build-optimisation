...
def aggregate_file_metrics_for_target(df: pd.DataFrame) -> dict:
    ...
    result = {
        ...
        "preprocessed_bytes_mean": float(df["preprocessed_bytes"].mean()) if not df["preprocessed_bytes"].isna().all() else 0.0,
        "expansion_ratio_mean": float(df["expansion_ratio"].mean()) if not df["expansion_ratio"].isna().all() else 0.0,
        # Object file metrics
