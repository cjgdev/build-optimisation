...
def parse_reply(build_dir: Path, source_dir: Path) -> CodeModel:
    ...
    if not reply_dir.is_dir():
        raise FileAPIError(
            f"No File API reply directory at {reply_dir}. Ensure cmake was run after query files were created."
        )
...
def _parse_target(data: dict, source_dir: str, build_dir: str) -> Target:
    ...
    # Parse artifacts
    artifacts = tuple(Artifact(path=_canonicalise(a["path"], build_dir)) for a in data.get("artifacts", []))
    ...
    link_frags = tuple(f.get("fragment", "") for f in link_data.get("commandFragments", []))
